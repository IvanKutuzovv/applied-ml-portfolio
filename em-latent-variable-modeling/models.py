...
class WordAligner(BaseAligner):
    ...
    def _e_step(self, parallel_corpus: List[TokenizedSentencePair]) -> List[np.array]:
        ...
        """
        posteriors = []
        for s in parallel_corpus:

            theta = self.translation_probs[s.source_tokens[:, None], s.target_tokens[None, :]]
            posteriors.append(theta / theta.sum(0, keepdims=True))  
    ...
    def _compute_elbo(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]) -> float:
        ...
        """
        elbo = 0.0
        for s, q in zip(parallel_corpus, posteriors):
            n = len(s.source_tokens)
            theta = self.translation_probs[s.source_tokens[:, None], s.target_tokens[None, :]]
            
            elbo += (q * (np.log(np.maximum(theta, 1e-20)) - np.log(n) - np.log(q + 1e-20))).sum()
    ...
    def _m_step(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]):
        ...
        """
        self.translation_probs = None
        counts = np.zeros((self.num_source_words, self.num_target_words), dtype=np.float32)
        for s, q in zip(parallel_corpus, posteriors):
            np.add.at(counts, (s.source_tokens[:, None], s.target_tokens[None, :]), q)
        norm = counts.sum(1, keepdims=True)
        norm[norm == 0] = 1
        counts /= norm
        self.translation_probs = counts
    ...
    def align(self, sentences):
        ...
        result = []
        for s, q in zip(sentences, self._e_step(sentences)):
            best = q.argmax(0) 
            result.append([(int(best[k]) + 1, k + 1) for k in range(len(s.target_tokens))])
        return result
...
class WordPositionAligner(WordAligner):
    ...
    def _e_step(self, parallel_corpus):
        posteriors = []
        for s in parallel_corpus:
            n, m = len(s.source_tokens), len(s.target_tokens)
            theta = self.translation_probs[s.source_tokens[:, None], s.target_tokens[None, :]]
            phi = self._get_probs_for_lengths(n, m)
            prod = theta * phi                              
            posteriors.append(prod / prod.sum(0, keepdims=True))
        return posteriors
    ...
    def _compute_elbo(self, parallel_corpus, posteriors):
        ...
        for s, q in zip(parallel_corpus, posteriors):
            ...
            theta = self.translation_probs[s.source_tokens[:, None], s.target_tokens[None, :]]
            phi = self._get_probs_for_lengths(n, m)
            elbo += (q * (np.log(np.maximum(theta, 1e-20))
                          + np.log(np.maximum(phi, 1e-20))
                          - np.log(q + 1e-20))).sum()
        return elbo
    ...
    def _m_step(self, parallel_corpus, posteriors):
        ...
        for s, q in zip(parallel_corpus, posteriors):
            np.add.at(counts, (s.source_tokens[:, None], s.target_tokens[None, :]), q)
            key = (len(s.source_tokens), len(s.target_tokens))
            if key not in phi_sum:
                phi_sum[key] = np.zeros(key, dtype=np.float32)
                phi_cnt[key] = 0
            phi_sum[key] += q
            phi_cnt[key] += 1
        norm = counts.sum(1, keepdims=True)
        ...
        self.translation_probs = counts
        for key in phi_sum:
            self.alignment_probs[key] = phi_sum[key] / phi_cnt[key]
        return self._compute_elbo(parallel_corpus, posteriors)