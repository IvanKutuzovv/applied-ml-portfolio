...
def compute_recall(reference: List[LabeledAlignment], predicted: List[List[Tuple[int, int]]]) -> Tuple[int, int]:
    ...
    """
    intersection, total_sure = 0, 0
    for ref, pred in zip(reference, predicted):
        sure = set(ref.sure)
        intersection += len(set(pred) & sure)
        total_sure += len(sure)
...
def compute_aer(reference: List[LabeledAlignment], predicted: List[List[Tuple[int, int]]]) -> float:
    ...
    """
    p_inter, p_total = compute_precision(reference, predicted)
    r_inter, r_total = compute_recall(reference, predicted)
    return 1 - (p_inter + r_inter) / (p_total + r_total)
